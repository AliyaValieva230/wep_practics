...
class ForbiddenException extends ApiException {
  const ForbiddenException([super.message = 'Недостаточно прав.']);
}
...
class ServerException extends ApiException {
  const ServerException([super.message = 'Ошибка на сервере.']);
}
...
ApiException mapHttpError(int status, dynamic body) {
  ...
  return switch (status) {
    ...
    _ => ServerException(message ?? 'Неизвестная ошибка ($status).'),
  };
}
...
ApiException mapDioError(DioException e) {
  ...
  if (status != null && status >= 400 && status < 500) {
    return mapHttpError(status, response?.data);
  }
  ...
  return switch (e.type) {
    ...
    DioExceptionType.connectionError => const NetworkException(
        'Не удалось соединиться с сервером. Проверьте консоль браузера (CORS).'),
