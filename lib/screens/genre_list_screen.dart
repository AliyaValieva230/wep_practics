...
class _GenreListScreenState extends State<GenreListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      ...
      child: Consumer<GenreListNotifier>(
  ...
  Widget _buildBody(
      BuildContext context, GenreListNotifier notifier, bool canEdit) {
    ...
    if (isCompact) {
      return EntityCardList<Genre>(
        items: notifier.result.items,
        idOf: (g) => g.id,
        selected: notifier.selected,
        onToggleSelect: notifier.toggleSelection,
        cardBuilder: (g) => Column(
          ...
          children: [
            ...
            if (g.description != null)
              Text(g.description!, overflow: TextOverflow.ellipsis),
          ],
        ),
        actions: (g) => [
          if (canEdit)
    ...
    return EntityTable<Genre>(
      ...
      columns: [
        TableColumnSpec(
            label: 'Название',
            sortField: 'name',
            build: (g) => Text(g.name, overflow: TextOverflow.ellipsis)),
        TableColumnSpec(
            label: 'Описание',
            build: (g) =>
                Text(g.description ?? '-', overflow: TextOverflow.ellipsis)),
  ...
  Widget _buildPagination(BuildContext context, GenreListNotifier notifier) {
