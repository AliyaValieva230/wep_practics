...
class _PublisherListScreenState extends State<PublisherListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      ...
      child: Consumer<PublisherListNotifier>(
  ...
  Widget _buildBody(
      BuildContext context, PublisherListNotifier notifier, bool canEdit) {
    ...
    if (notifier.result.items.isEmpty) {
      return const Center(child: Text('Издательства не найдены'));
    }

    final isCompact = MediaQuery.sizeOf(context).width < 600;

    if (isCompact) {
      return EntityCardList<Publisher>(
        ...
        cardBuilder: (p) => Column(
          ...
          children: [
            ...
            if (p.address != null)
              Text(p.address!, overflow: TextOverflow.ellipsis),
          ],
        ),
        actions: (p) => [
          if (canEdit)
    ...
    return EntityTable<Publisher>(
      ...
      columns: [
        TableColumnSpec(
            label: 'Название',
            sortField: 'name',
            build: (p) => Text(p.name, overflow: TextOverflow.ellipsis)),
        TableColumnSpec(
            label: 'Адрес',
            build: (p) =>
                Text(p.address ?? '-', overflow: TextOverflow.ellipsis)),
  ...
  Widget _buildPagination(
      BuildContext context, PublisherListNotifier notifier) {
